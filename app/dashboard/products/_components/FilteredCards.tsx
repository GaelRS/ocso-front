...
import ProductCard from "../../providers/_components/ProductCard";
...
export default function FilteredCards({products}:{products: Product[]}){
   ...
   return(
      <div className="max-h-[90vh] min-h-[90vh] overflow-y-auto flex flex-col gap-8 border-r-orange-400 border-r-2 pt-10 px-10">
         <Input label="Nombre del producto" onChange={(e)=>{
            setFiltered(e.target.value)
         }}/>
         {
            productsList.map((product)=>{
               return(
                  <Link className="hover:scale-110 transition-transfom" key={product.productId} href={{pathname: `/dashboard/products/${product.productId}`} } >
